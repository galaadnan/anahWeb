...
EXPLICIT_RULES = {
    ...
    "منقهر": "غاضب", "مفور": "غاضب", "منفعل": "غاضب",

    # 😊 كلمات السعادة
    "سعيد": "سعيد", "فرحان": "سعيد", "مبسوط": "سعيد", 
    "مستانس": "سعيد", "طاير": "سعيد", "مبهوج": "سعيد",

    # 😢 كلمات الحزن
    "حزين": "حزين", "زعلان": "حزين", "متضايق": "حزين", 
    "مكتئب": "حزين", "مهموم": "حزين", "مقهور": "حزين",

    # 🥱 كلمات التعب
    "تعبان": "تعبان", "مرهق": "تعبان", "هلكان": "تعبان", 
    "مهدود": "تعبان", "طافي": "تعبان", "دايخ": "تعبان", "مكسر": "تعبان",

    # 😰 كلمات التوتر
    "متوتر": "متوتر", "قلقان": "متوتر", "مرتبك": "متوتر", 
    "خايف": "متوتر", "مخبوص": "متوتر", "مشغول": "متوتر",

    # 😌 كلمات الهدوء
    "هادئ": "هادئ", "رايق": "هادئ", "مروق": "هادئ", 
    "مرتاح": "هادئ", "مسترخي": "هادئ", "مفضي": "هادئ"
}
...
def query_model(text_list):
    ...
    try:
        for text in text_list:
            ...
            # إذا كانت الجملة قصيرة (3 كلمات أو أقل عشان نستوعب العامية أكثر)
            if len(words) <= 3:
                for word in words:
                    # تنظيف الكلمة من الحروف الزائدة (مثل: معععصب، زعلانن) لو حبيتي مستقبلاً
                    if word in EXPLICIT_RULES:
                        matched_label = EXPLICIT_RULES[word]
                        break
            ...
            # 2. التحليل العميق بالموديل للجمل الطويلة والمعقدة
